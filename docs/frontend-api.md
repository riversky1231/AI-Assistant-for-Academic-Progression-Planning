...
当前前端已使用登录、注册、找回密码、微信登录、资料维护、改密码、管理员账号管理和智能咨询接口；没有调用收藏、档案云同步等接口。

## 智能咨询页面

`pages/chat/index` 调用 `POST /agent/chat`，请求仅包含 `message` 和续聊时的 `conversation_id`。消息最长 4000 字符单位，单独设置 300 秒超时，响应为完整 JSON。首次省略会话 ID，后续原样使用成功响应返回的 ID。档案信息先填入草稿，经用户点击发送后才进入请求。

`pages/chat-sources/index` 使用当前回答的内存快照，按 `sources[].tool` 展示院校、历史录取、冲稳保和背景资料，不额外发起 Agent 查询。没有依据时允许正常展示回答或追问，背景资料不标为实时数据。

咨询的 409 表示同一账号有请求处理中；404 结合服务端消息区分会话过期与院校不存在。会话过期保留问题，由用户开始新咨询。429、502、503、504 和网络失败均不自动重试。401 仍清理账号数据并返回登录，成功登录回到咨询入口。请求期间离开页面不取消服务端分析；退出或换号后迟到回复会被丢弃。

完整字段、会话生命周期及后端配置见 [Agent 接口文档](Agent接口文档.md)。
